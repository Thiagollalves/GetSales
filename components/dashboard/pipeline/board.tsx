...
export default function PipelineBoard() {
  ...
  const handleRenameStage = (stageId: string) => {
    ...
    setStages((prev) => prev.map((s) => (s.id === stageId ? { ...s, title: trimmed } : s)))
  ...
  const getVisibleLeads = (stage: Stage) =>
    normalizedSearch ? stage.leads.filter(matchesLead) : stage.leads

  const totalLeads = stages.reduce((sum, stage) => sum + getVisibleLeads(stage).length, 0)
  const totalValue = stages.reduce(
    (sum, stage) =>
      sum +
      getVisibleLeads(stage).reduce((valueSum, lead) => valueSum + lead.value, 0),
    0
  )
  const editingStage = editingStageId
    ? stages.find((stage) => stage.id === editingStageId)
    : undefined
        ...
        <Card className="mx-4 mt-4 border border-border/50 bg-card/60 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Etapa: {editingStage?.title ?? "—"}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(event) => setLeadForm((prev) => ({ ...prev, lastContact: event.target.value }))}
      ...
      >
        <div className="flex-1 overflow-x-auto p-4 lg:p-6">
          <div className="flex gap-4 h-full min-w-max">
            {stages.map((stage) => {
              const stageLeads = getVisibleLeads(stage)
              return (
                <div
                  key={stage.id}
                  className={`
                ...
                >
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        <DropdownMenuItem onSelect={() => handleRenameStage(stage.id)}>Renomear etapa</DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => handleRemoveStage(stage.id)}>Excluir etapa</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
                  <div className="px-4 pb-3">
                    <p className="text-xs text-muted-foreground">
                      ...
                      <span className="font-semibold text-foreground">
                        {formatCurrency(stageLeads.reduce((sum, lead) => sum + lead.value, 0))}
                      </span>
                    </p>
                  </div>
                  <div className="flex-1 overflow-y-auto px-3 pb-3">
                    ...
                    >
                      {stageLeads.map((lead) => (
                        <SortableLead key={lead.id} lead={lead} onEdit={() => handleEditLead(lead, stage.id)} />
                      ))}
                    </SortableContext>
                    <Button variant="ghost" className="w-full mt-2 border border-dashed border-border h-10" onClick={() => handleAddLead(stage.id)}>
